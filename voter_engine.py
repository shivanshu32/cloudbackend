...
class VoterInstance:
    ...
    def __init__(self, proxy_ip: str, proxy_config: dict, instance_id: int, target_url: str, voter_manager=None, vote_logger=None):
        ...
        self.session_dir = os.path.join(backend_dir, f"brightdata_session_data/instance_{instance_id}")
        ...
        if vote_logger:
            ...
            backend_dir = os.path.dirname(os.path.abspath(__file__))
            log_file_path = os.path.join(backend_dir, "voting_logs.csv")
    ...
    async def _initialize_browser(self, use_session=False):
        ...
        try:
            ...
            async def _do_init():
                ...
                # HEADLESS DISABLED FOR LOCAL DEBUGGING
                self.browser = await self.playwright.chromium.launch(
                    headless=True,  # Changed from True to False for debugging
                    proxy={
    ...
    async def clear_popups_enhanced(self):
        ...
        try:
            ...
            # PHASE 1: Check for visible popup containers and try to hide them
            try:
                # Check if popmake popup is visible
                popup_containers = await self.page.query_selector_all('div.pum-container.popmake[style*="display: block"]')
                if popup_containers:
                    logger.info(f"[POPUP] Found {len(popup_containers)} visible popup containers")
                    # Try to hide them with JavaScript
                    await self.page.evaluate('''() => {
                        const popups = document.querySelectorAll('div.pum-container.popmake');
                        popups.forEach(popup => {
                            popup.style.display = 'none';
                            popup.style.opacity = '0';
                            popup.style.visibility = 'hidden';
                        });
                        // Also remove overlay if exists
                        const overlays = document.querySelectorAll('.pum-overlay');
                        overlays.forEach(overlay => {
                            overlay.style.display = 'none';
                        });
                    }''')
                    logger.info(f"[POPUP] Hidden popup containers with JavaScript")
                    await asyncio.sleep(0.2)
            except Exception as e:
                logger.debug(f"[POPUP] Could not hide popup containers: {e}")
            
            # PHASE 2: Escape sequences to dismiss popups
            for i in range(4):
                await self.page.keyboard.press('Escape')
                await asyncio.sleep(0.05)
            
            # PHASE 3: Specific popup close buttons (cutebabyvote.com specific)
            specific_selectors = [
                # Specific popup ID selectors (for popmake-9794 and similar)
                '#popmake-9794 button.pum-close.popmake-close',
                'div[id^="popmake-"] button.pum-close.popmake-close',
                'div.pum-container button.pum-close.popmake-close',
                # Generic popmake selectors
            ...
            # PHASE 5: Final escape sequences
            for i in range(2):
                await self.page.keyboard.press('Escape')
                await asyncio.sleep(0.05)
