...
# Vote logger - use backend directory (same as app.py)
backend_dir = os.path.dirname(os.path.abspath(__file__))
vote_logger_path = os.path.join(backend_dir, "voting_logs.csv")
...
def config_endpoint():
    ...
    # Path to persistent config file (in backend directory)
    config_file = os.path.join(backend_dir, 'user_config.json')
...
def start_monitoring():
    ...
    try:
        ...
        # Load saved config if available (in backend directory)
        config_file = os.path.join(backend_dir, 'user_config.json')
        saved_config = {}
        if os.path.exists(config_file):
            try:
...
def restart_system():
    ...
    try:
        ...
        # Load saved config (in backend directory)
        config_file = os.path.join(backend_dir, 'user_config.json')
        saved_config = {}
        if os.path.exists(config_file):
            try:
...
@app.route('/api/voting-statistics', methods=['GET'])
def get_voting_statistics():
    """Get voting statistics from CSV logs with all instances and hours"""
    try:
        import csv
        import os
        ...
        if not os.path.exists(csv_file):
            return jsonify({
                'status': 'error',
                'message': 'Voting logs file not found',
                'data': {'logs': [], 'instances': [], 'hours': []}
            })
        ...
        with open(csv_file, 'r', encoding='utf-8') as f:
            ...
            logger.info(f"[STATS] CSV columns: {reader.fieldnames}")
            ...
            for row in reader:
                ...
                logs.append({
                    ...
                    'instance_id': row.get('instance_id', row.get('instance_name', '')),
                    'status': row.get('status', ''),
                    'failure_type': row.get('failure_type', ''),
                    'initial_count': row.get('initial_vote_count', row.get('vote_count', '')),
                    'final_count': row.get('final_vote_count', ''),
                    'proxy_ip': row.get('proxy_ip', row.get('ip', '')),
                    'session_id': row.get('session_id', ''),
                    'failure_reason': row.get('failure_reason', row.get('message', '')),
                    'cooldown_message': row.get('cooldown_message', '')
                })
        
        logger.info(f"[STATS] Read {row_count} rows from CSV, parsed {len(logs)} logs")
        
        # Sort by timestamp (newest first)
        logs.sort(key=lambda x: x['timestamp'], reverse=True)
        
        # Get unique instances from brightdata_session_data folder
        instances = []
        backend_dir = os.path.dirname(os.path.abspath(__file__))
        session_data_dir = os.path.join(backend_dir, 'brightdata_session_data')
        if os.path.exists(session_data_dir):
            for item in os.listdir(session_data_dir):
                if os.path.isdir(os.path.join(session_data_dir, item)) and item.startswith('instance_'):
                    instance_num = item.replace('instance_', '')
                    instances.append(f'Instance_{instance_num}')
        
        # Sort instances numerically
        instances.sort(key=lambda x: int(x.replace('Instance_', '')))
        
        # Get unique hours from logs
        hours = set()
        for log in logs:
            try:
                dt = datetime.fromisoformat(log['timestamp'])
                hour_key = f"{dt.year}-{dt.month:02d}-{dt.day:02d} {dt.hour:02d}:00"
                hours.add(hour_key)
            except:
                continue
        
        hours = sorted(list(hours), reverse=True)
...
def get_saved_sessions():
    ...
    try:
        sessions = []
        backend_dir = os.path.dirname(os.path.abspath(__file__))
        session_dir = os.path.join(backend_dir, "brightdata_session_data")
                    ...
                    try:
                        ...
                        if os.path.exists(info_file):
                            ...
                            try:
                                ...
                                backend_dir = os.path.dirname(os.path.abspath(__file__))
                                csv_file = os.path.join(backend_dir, 'voting_logs.csv')
    ...
    try:
        ...
        async def open_login_browser():
            try:
                ...
                backend_dir = os.path.dirname(os.path.abspath(__file__))
                session_dir = os.path.join(backend_dir, "brightdata_session_data", f"instance_{instance_id}")
...
async def check_ready_instances():
    ...
    try:
        ...
        backend_dir = os.path.dirname(os.path.abspath(__file__))
        session_dir = os.path.join(backend_dir, "brightdata_session_data")
        log_file = os.path.join(backend_dir, "voting_logs.csv")
